...
// Consts
import { options, BASE_URL, APIKEY, WATCHLIST_KEY } from "../constant.js"
...
function Movies() {
  ...
  const setWatchlist = (movieObj) => {
    ...
    value = [...value, { title: movieObj.title||movieObj.name, id: movieObj.id,path:movieObj.poster_path},]
          ...
          {movies.map((movie, index) => {
            ...
            return (
              <div key={index}>
                ...
                <div><button onClick={()=>setWatchlist(movie)}>❤️</button></div>
              </div>
            )
          })}
        </div>
